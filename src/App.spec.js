...
test('Test that main component renders', async () => {
    mockFetchShow.mockResolvedValueOnce(episodeData)
    const {rerender, queryByTestId} = render(<App />)
    waitFor(() => {
      expect(queryByTestId('section')).toBeTruthy()
    })
  })
  test('Test that Stranger Things title renders', async () => {
    mockFetchShow.mockResolvedValueOnce(episodeData)
    const {rerender, queryAllByText} = render(<App />)
    const title = await queryAllByText('Stranger Things')
